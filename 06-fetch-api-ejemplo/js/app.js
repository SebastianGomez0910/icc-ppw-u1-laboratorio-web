...
function renderizar(lista) {
  ...
  lista.forEach(item => {
    ...
    //REVVISAR ESTO
    ocupacion.className = 'frases';

    // Validar si existen frases
    if (item.phrases && item.phrases.length > 0) {
      // Tomar solo las 3 primeras
      const primerasFrases = item.phrases.slice(0, 3);

      // Crear un <p> por cada frase
      primerasFrases.forEach(frase => {
        const p = document.createElement('p');
        p.textContent = frase;
        ocupacion.appendChild(p);
      });
    } else {
      // Si no hay frases, mostrar un mensaje
      const p = document.createElement('p');
      p.textContent = 'No hay frases disponibles';
      ocupacion.appendChild(p);
    }
