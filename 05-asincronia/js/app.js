...
function mostrarLogError(mensaje, tipo = 'info'){
  const item = document.createElement('div');
  item.className = `log-item log-${tipo}`;
  item.textContent = `[${new Date().toLocaleTimeString()}] ${mensaje}`;  logErrores.appendChild(item);
...
async function fetchConReintentos(nombre, intentos = 3) {
  ...
  for(let i=0; i<intentos; i++){
    try{
      ...
      mostrarLogError(`Exito en intento ${i+1}: ${nombre} cargando`, 'success');
      return resultado;
    }
    catch(error){
      mostrarLogError(`Intento ${i+1} fallo: ${error.message}`, 'error');
    ...
    throw new Error(`No se puede cargar ${nombre} despues de ${intentos} intentos`);
}
